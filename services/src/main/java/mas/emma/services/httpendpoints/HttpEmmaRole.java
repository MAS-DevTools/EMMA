...
public class HttpEmmaRole {

    private String base = Application.getSettings().getServerUrl() + ControllerConstants.ROLEPATH;

    //<editor-fold defaultstate="collapsed" desc="CREATE">
    public <S extends EmmaRole> EmmaRole create(String user_id, S s) {
        String uri = base + ControllerConstants.CREATE;
        return new EmmaRole().valueOf(Application.clientHandlerInstance().getEntity(uri, user_id, HttpMethod.POST, new StringEntity(s.toJson(), ContentType.APPLICATION_JSON)));
    }

    //</editor-fold>
    //<editor-fold defaultstate="collapsed" desc="READ">
    public EmmaRole getById(String user_id, String id) {
        String uri = base + ControllerConstants.GET_BY_ID2 + id;
        return new EmmaRole().valueOf(Application.clientHandlerInstance().getEntity(uri, user_id, HttpMethod.GET, null));
    }

    //</editor-fold>
    //<editor-fold defaultstate="collapsed" desc="UPDATE">
    public <S extends EmmaRole> EmmaRole update(String user_id, Iterable<S> s) {
        String uri = base + ControllerConstants.UPDATE;
        return new EmmaRole().valueOf(Application.clientHandlerInstance().getEntity(uri, user_id, HttpMethod.PUT, new StringEntity(s.toJson(), ContentType.APPLICATION_JSON)));
    }
