...
import mas.emma.webapi.services.TopicGroupService;
 ...
 */
@RestController
@RequestMapping(ControllerConstants.TOPICGROUPPATH)
public class TopicGroupController {
    ...
    public <S extends TopicGroup> ResponseEntity<Object> create(@RequestHeader(required = true) MultiValueMap<String, String> headers, @RequestBody(required = true) S s) {
    ...
    public <S extends TopicGroup> ResponseEntity update(@RequestHeader(required = true) MultiValueMap<String, String> headers, @RequestBody(required = true) S s) {
        return runWithErrorHandeling(() -> {
